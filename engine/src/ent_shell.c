...
/*   Updated: 2022/11/24 23:20:53 by alde-fre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_ft_shell_display(t_entity *self, t_data *game)
{
	...
	ft_put_sprite_r(game->eng, game->spr[16 + anim], ft_v2i(self->pos.x
			- game->cam.pos.x, self->pos.y - game->cam.pos.y - 11),
		ft_v2i(4, 4), self->rot);
	return (1);
}
...
static int	_ft_shell_update(t_entity *self, t_data *game, float dt)
{
	...
	while (i < ft_vector_size(game->map.entities))
	{
		...
		if (ent != self && ent->type == 2)
			if (ft_v2fmag(ft_v2fsub(ent->pos, self->pos)) < 15)
			{
